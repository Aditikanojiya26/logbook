...
const SectionTimeLimit = require("../backend/models/SectionTimeLimit");
...
const unit = require("../backend/models/unit");
const parameterAuxiliarySCE = require("../backend/models/parameterAuxiliarySCE.js");
async function initDatabase() {
  try {
    ...
    const sectionLimits = [
      {
          section: "shiftBeginning",
          relativeStartMinutes: 0,
          relativeEndMinutes: 60
      },
      {
          section: "shiftMidNight",
          relativeStartMinutes: 180,
          relativeEndMinutes: 240
      },
      {
          section: "shiftEnd",
          relativeStartMinutes: 420,
          relativeEndMinutes: 480
      }
    ];
    await SectionTimeLimit.deleteMany();
    await SectionTimeLimit.insertMany(sectionLimits);
    console.log("Section time limits inserted successfully!");
...
await Auxiliary.deleteMany();
    ...
    for (let auxiliary of aux) {
      ...
      await parameterAuxiliarySCE.deleteMany();
    const parAux=[
      {
        name: "Coal Mills",
        options: [
    ...
    await parameterAuxiliarySCE.insertMany(parAux);
    console.log("parameter Auxiliary inserted successfully!");
