...
const validateSectionOrder = async (shiftData, section) => {
  const sectionOrder = ["shiftBeginning", "shiftEnd","midNight", "operationPerformed"];
...
const submitSection = async (req, res) => {
  try {
    ...
    if (section === "midNight" && shift.name !== "Night Shift") {
      return res.status(400).json({ message: "Midnight section can only be submitted during Night shift." });
    }
    ...
    const shiftStart = new Date(shift.time); // e.g. 2025-04-20T22:00:00
    let shiftEndTime = new Date(shiftStart);

    shiftEndTime.setHours(shiftEndTime.getHours() + 8); // ends at 5:00 AM next day
...
console.log(section)
    if (!shiftData) {
      ...
      shiftData = new ShiftData({
        ...
        unitId,
        shiftId,
        shiftBeginning: { parameters: parameters, inServiceAuxiliaries: inServiceAuxiliaries }
      });
